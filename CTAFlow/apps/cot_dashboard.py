"""Dash application for exploring COT features generated by :class:`COTAnalyzer`.

The dashboard exposes controls for selecting a ticker and analysis date range and
visualizes each feature group returned by :func:`CTAFlow.features.signals_processing.COTAnalyzer.calculate_enhanced_cot_features`.
        ...
        try:
            if TICKER_TO_CODE:
                ...
                if cot_code:
                    ...
                    if raw_df is not None and not raw_df.empty:
                        # Use COTAnalyzer's built-in cleaning first
...
def compute_feature_groups(df: pd.DataFrame) -> OrderedDict[str, pd.DataFrame]:
    """Compute feature DataFrames for each COTAnalyzer group."""

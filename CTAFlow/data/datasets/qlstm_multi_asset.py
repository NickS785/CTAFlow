...
class MultiAssetDistributionDataset(Dataset):
    ...
    The model predicts the PDF of the **next single-day return** r_{t+1}.
    The target is a scalar, normalised by today's group EWMA vol sigma_t
    (known at prediction time — no look-ahead bias).

    Output keys:
      - asset_features   : [seq_len, F_asset]
      - market_features  : [seq_len, F_market]
      - asset_class_id   : scalar long
      - raw_returns      : scalar float  — r_{t+1}
      - norm_returns     : scalar float  — r_{t+1} / sigma_t
      - group_vol        : scalar float  — sigma_t
    ...
    def __init__(
        ...
        # Need at least max_seq lookback + 1 for the next-day target
        ...
        for ticker, feat_df in asset_features.items():
            ...
            # Only need t+1 to exist for the scalar next-day target
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # --- scalar target (paper methodology) ---
        # r_{t+1}: next single-day log return
        raw_ret = data["returns"][t + 1]
        # sigma_t: today's group EWMA vol — known at prediction time, no look-ahead
        gvol = data["group_vol"][t]
        # normalised target: r_{t+1} / sigma_t
        ...
        return {
            ...
            "raw_returns": torch.tensor(raw_ret, dtype=torch.float32),
            "norm_returns": torch.tensor(norm_ret, dtype=torch.float32),
...
def collate_variable_seq(batch: List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
    ...
    # Scalar targets — (B,) each
    raw_returns = torch.zeros(batch_size)
    norm_returns = torch.zeros(batch_size)
    group_vols = torch.zeros(batch_size)
    ...
    return {
        ...
        "raw_returns": raw_returns,          # (B,) scalar
        "norm_returns": norm_returns,         # (B,) scalar
        "group_vol": group_vols,              # (B,) scalar
...
def build_full_pipeline(
    ...
    Target is the scalar next-day return r_{t+1}, normalised by today's
    group EWMA vol sigma_t.  No forecast_horizon parameter — the model
    predicts the PDF of a single return.
    """

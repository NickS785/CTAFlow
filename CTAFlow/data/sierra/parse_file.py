...
class AsyncScidReader:
    """
    Asynchronous SCID reader for high-performance batch processing.

    Provides async methods for reading multiple SCID files concurrently
    and batch processing operations.
    """

    def __init__(self, max_workers: Optional[int] = None):
        """
        Initialize async SCID reader.
    ...
    def _read_scid_sync(self, path: str, **kwargs) -> pd.DataFrame:
        """Synchronous SCID reading wrapper for thread execution."""
        try:
            with ScidReader(path) as reader:
                return reader.to_dataframe(**kwargs)
        except Exception as e:
            self.logger.error(f"Error reading {path}: {e}")
            return pd.DataFrame()

    async def read_scid_async(self, path: str, **kwargs) -> pd.DataFrame:
        """
        Asynchronously read a single SCID file.

        Args:
            path: Path to SCID file
            **kwargs: Arguments passed to ScidReader.to_dataframe()

        Returns:
            DataFrame with SCID data
        """
        loop = asyncio.get_event_loop()
        return await loop.run_in_executor(
            self.executor,
            self._read_scid_sync,
            path,
            **kwargs
        )

    async def read_multiple_scid(self,
                                file_paths: List[str],
                                batch_size: int = 10,
                                **kwargs) -> List[Tuple[str, pd.DataFrame]]:
        """
        Read multiple SCID files concurrently.

        Args:
            file_paths: List of SCID file paths
        ...
        for i in range(0, len(file_paths), batch_size):
            ...
            tasks = [
                self.read_scid_async(file_path, **kwargs)
            ...
            for file_path, result in zip(batch, batch_results):
                if isinstance(result, Exception):
                    ...
                    results.append((file_path, pd.DataFrame()))
                else:
                    results.append((file_path, result))

        return results

    async def read_and_aggregate(self,
                                file_paths: List[str],
                                aggregate_func: Optional[callable] = None,
                                batch_size: int = 10,
                                **kwargs) -> pd.DataFrame:
        """
        Read multiple SCID files and aggregate them.

        Args:
            file_paths: List of SCID file paths
            aggregate_func: Custom aggregation function (default: concatenate)
        ...
        """
        # Read all files
        file_results = await self.read_multiple_scid(file_paths, batch_size, **kwargs)
        ...
        # Apply aggregation
        if aggregate_func:
            return aggregate_func(dataframes)
        else:
            # Default: concatenate and sort by index
            return pd.concat(dataframes, axis=0).sort_index()

    def _peek_scid_sync(self, path: str, n: int = 3) -> dict:
        """Synchronous SCID peek wrapper for thread execution."""
        try:
            with ScidReader(path) as reader:
    ...
    async def peek_multiple_scid(self,
                                file_paths: List[str],
                                n: int = 3,
                                batch_size: int = 20) -> Dict[str, dict]:
        """
        Peek at multiple SCID files to get basic information.

        Args:
            file_paths: List of SCID file paths
            n: Number of records to peek at
            batch_size: Batch size for concurrent processing

        Returns:
            Dictionary mapping file paths to peek results
        """
        results = {}
    ...
    async def validate_multiple_scid(self,
                                   file_paths: List[str],
                                   batch_size: int = 20) -> Dict[str, dict]:
        """
        Validate multiple SCID files asynchronously.

        Args:
            file_paths: List of SCID file paths to validate
            batch_size: Batch size for concurrent processing
        ...
        # Get basic info for all files
        peek_results = await self.peek_multiple_scid(file_paths, n=1, batch_size=batch_size)

        for file_path, peek_result in peek_results.items():
            file_result = {
                'valid': False,
                'error': None,
                'file_size_mb': 0,
            ...
            try:
                ...
                file_result['file_size_mb'] = file_size / (1024 * 1024)

                if 'error' in peek_result:
                    file_result['error'] = peek_result['error']
                else:
                    file_result['valid'] = True
                    file_result['records'] = peek_result.get('n_records', 0)

                    # Extract variant info if available
                    if peek_result.get('int64_us_first'):
                        file_result['variant'] = 'int64_us'
                    elif peek_result.get('double_days_first'):
                        file_result['variant'] = 'double_days'
...
async def read_scid_async(
    ...
    """
    Asynchronous one-shot helper for reading SCID files.
    """
    async with AsyncScidReader(max_workers=1) as reader:
        return await reader.read_scid_async(
            path,
            start=start,
            end=end,
            fields=fields,
...
async def read_multiple_scid_async(
    file_paths: List[str],
    batch_size: int = 10,
    start: Optional[pd.Timestamp] = None,
    end: Optional[pd.Timestamp] = None,
    ...
    """
    Asynchronously read multiple SCID files.

    Returns:
        List of (file_path, dataframe) tuples
    """
    async with AsyncScidReader() as reader:
        return await reader.read_multiple_scid(
            file_paths,

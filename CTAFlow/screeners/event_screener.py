...
Renamed from :mod:`data_release_screener` with optional orderflow integration.
"""
...
def run_event_screener(
    ...
    *,
    ...
    Optionally enriches events with orderflow summaries near each release when
    ``params.use_orderflow`` is enabled.
    """
    ...
    def _summarize_orderflow(window: pd.DataFrame) -> Dict[str, float]:
        if window.empty:
            return {}
        cols = {}
        if "side" in window.columns:
            side = window["side"].replace({"buy": 1, "sell": -1, "B": 1, "S": -1}).astype(float)
            size = window.get("size") or window.get("qty") or window.get("volume")
            if size is None:
                return {}
            size = pd.to_numeric(size, errors="coerce")
            signed = side * size
            cols["of_signed_vol"] = float(signed.sum())
            total = float(size.sum())
            cols["of_total_vol"] = total
            cols["of_imbalance"] = float(signed.sum() / total) if total else np.nan
            cols["of_large_trade_vol_frac"] = float((size[size > size.median() * 2].sum() / total)) if total else np.nan
        elif {"buy_vol", "sell_vol"}.issubset(window.columns):
            buy = pd.to_numeric(window["buy_vol"], errors="coerce")
            sell = pd.to_numeric(window["sell_vol"], errors="coerce")
            signed = buy - sell
            total = (buy + sell).sum()
            cols["of_signed_vol"] = float(signed.sum())
            cols["of_total_vol"] = float(total)
            cols["of_imbalance"] = float(signed.sum() / total) if total else np.nan
        return cols
    ...
    for _, ev in events.iterrows():
        ...
        if params.use_orderflow and orderflow is not None:
            window_start = rel_ts - timedelta(minutes=params.orderflow_window_pre_minutes)
            window_end = rel_ts + timedelta(minutes=params.orderflow_window_post_minutes)
            of = orderflow.copy()
            ts_col = "ts" if "ts" in of.columns else "bucket_start" if "bucket_start" in of.columns else None
            if ts_col:
                of_ts = pd.to_datetime(of[ts_col])
                of_ts = _ensure_tz_aware(of_ts, instrument_tz)
                of[ts_col] = of_ts
                window = of[(of_ts >= window_start) & (of_ts <= window_end)]
                of_cols = _summarize_orderflow(window)
                row.update({f"{k}_event": v for k, v in of_cols.items()})
    ...
    if events_df.empty:
        return EventScreenerResult(events=events_df, summary=pd.DataFrame(), patterns=[])
    ...
    for (sym, code), g in grp:
        ...
        if params.use_orderflow:
            imbalance_col = "of_imbalance_event"
            if imbalance_col in g.columns:
                rho_flow_react, p_flow_react = _corr(g[imbalance_col], g["r_event"])
                rho_flow_T0, p_flow_T0 = _corr(g[imbalance_col], g["r_T0"])
                summary_row.update(
                    {
                        "rho_flow_event": rho_flow_react,
                        "p_rho_flow_event": p_flow_react,
                        "rho_flow_T0": rho_flow_T0,
                        "p_rho_flow_T0": p_flow_T0,
                    }
                )
                if abs(rho_flow_T0) >= params.corr_threshold:
                    pattern_type = "event_flow_trend_T0" if rho_flow_T0 > 0 else "event_flow_reversal_T0"
                    patterns.append(
                        {
                            "symbol": sym,
                            ...
                            "event_code": code,
                            "gate_direction": int(np.sign(rho_flow_T0)),
                            "strength": abs(rho_flow_T0),
                            "description": f"{code} flow/return correlation to T0 close",
                        }
                    )
    ...
    return EventScreenerResult(events=events_df, summary=summary_df, patterns=patterns)

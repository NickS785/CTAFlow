...
class HorizonMapper:
    ...
    def _collect_metadata_fields(
        ...
        for field in fields:
            candidates = alias_map.get(field, (field,))
            value: Any = None
            found = False
    ...
    def add_predictor_columns(
        ...
        *,
        ...
        if len(df) != result_length:
            raise HorizonInputError(
                "add_predictor_columns requires bars to align with the sanitised frame"
            )
        ...
        for gate_col, _, returns_x_series, _, _, _, _ in self._iter_pattern_returns(
            ...
            signal_values = np.full(result_length, np.nan, dtype=float)
            gate_mask = (df[gate_col] == 1).to_numpy()
            if gate_mask.any():
                aligned = returns_x_series.reindex(df.index)
                aligned_values = aligned.to_numpy(dtype=float, copy=False)
                valid_mask = gate_mask & np.isfinite(aligned_values)
                if valid_mask.any():
                    signal_values[valid_mask] = aligned_values[valid_mask]
            signal = pd.Series(signal_values, index=result.index, dtype=float)
